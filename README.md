...
Objective functions, defined as differences between calculated data (using pseudopotentials in QE) and known benchmark data (generally from all-electron calculations performed with WIEN2k, but may also be from well-converged pseudopotential calculations), are minimized for a given set of elements and compounds with respect to specified properties.

### General usage notes:

(1) Create a working directory under which all calculations will take place.

(2) Write the input.json file containing information on the compounds and properties to be tested. Several sample input files are provided in the Examples/ folder. Input files for entire optimization sets are also provided in the Optimization_Sets/ folder. The general structure is as follows:
- "directories":
...
(6) Once the optimization is complete, use scripts/get_best_soln.py to to retrieve the best solution obtained throughout all generations considered (from dakota_tabular.dat). The weighted sum approach is currently utilized to normalize all objective functions, for which the mean absolute error (MAE) is calculated. By default, all objectives are weighted equally. Alternatively, user specified weights may be used; to do so, add --weights to the end of the python execution statement. In comparing all result sets, the universal minimum in MAE is chosen as the best solution; corresponding objective functions and atompaw input files are placed in the Best_Solution/ folder. Note the get_best_soln.py script may also be used while the optimization is still running, assuming at least one generation has completed, which is useful to check the progress of your run (how fit is the current best solution).

(7) Once a set of optimized pseudopotentials is obtained, a final test may be carried out using scripts/test_PP.py. The procedure follows same as usual according to the input.json file, however, no dakota settings are necessary. Additionally, it is recommended that a new directory containing the .UPF files be created, for which the path is specified as "paw_dir" in the input.json (under "directories"). If this setting is not used, the PAW must be contained in the "cmpd_template_dir" path.
...
- plot_wfn.py (Usage: python plot_wfn.py *wavefunction number*, executed within directory containing AtomPAW output)
    - Visualize exact and pseudized wavefunctions as generated by AtomPAW
    
- plot_log.py (Usage: python plot_log.py, executed within directory containing AtomPAW output)
    - Visualize logarithmic derivatives of exact and pseudized for all wavefunctions, excluding unbound state
    
- plot_alog.py (Usage: python plot_alog.py, executed within directory containing AtomPAW output)
    - Visualize arctangent of the logarithmic derivatives of exact and pseudized for all wavefunctions, excluding unbound state
    - Arctangent is used to capture ghost states (unphysical discontinuities in the pseudized wavefunction derivatives)
    
- qe_reader.py (Usage: python qe_reader.py *name of QE output file*)
    - Read and print final lattice parameters from a QE run
