...
(7) To improve efficiency of the pseudopotential, copy the atompaw input file (from Best_Solution) into a working directory along with the corresponding input.json. Use scripts/optimize_loggrid.py to iteratively decrease the grid density until tolerances to changes in energy and lattice constants are reached. The default tolerance values are set to 10<sup>-6</sup> eV and 10<sup>-3</sup> angstroms, but may be changed by the user. After completion, the [elem].atompaw.in file will contain the optimal loggrid density.

(8) Once a set of optimized pseudopotentials is obtained, a final test may be carried out using scripts/test_PP.py. The procedure follows same as usual according to the input.json file, however, no dakota settings are necessary. Additionally, it is recommended that a new directory containing the .UPF files be created, for which the path is specified as "paw_dir" is the input.json (under "directories").
