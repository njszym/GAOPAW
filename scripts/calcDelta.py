...
def show_results(outfile, infile, reffile, Delta, Deltarel, Delta1, eloverlap,
    ...
    for el in systems:
        while True:
            try:
                ...
                el_str = eloverlap[i].decode('utf-8')
                outfile.write(el_str + '\t %.3f\t %.1f\t%.3f \n' 
    ...
    outfile.write('np.max   %.3f\t %.1f\t%.3f \t (%s, %s, %s)\n'
        % (Delta[Dmax], Deltarel[Drelmax], Delta1[D1max], eloverlap[Dmax].decode('utf-8'),
        eloverlap[Drelmax].decode('utf-8'), eloverlap[D1max].decode('utf-8')))
    outfile.write('np.min   %.3f\t %.1f\t%.3f \t (%s, %s, %s)\n'
        % (Delta[Dmin], Deltarel[Drelmin], Delta1[D1min], eloverlap[Dmin].decode('utf-8'),
        eloverlap[Drelmin].decode('utf-8'), eloverlap[D1min].decode('utf-8')))
    outfile.write('--------------------\n')
