...
## Also stop if QE runs into error, e.g., S matrix not positive definite.
## Tolerances may be changed depending on desired degree of accuracy.

## Usage: python optimize_loggrid.py elem_name
...
def main():
    ...
    while (ediff < etol) and (FCC_diff < FCC_tol) and (BCC_diff < BCC_tol) and check == True:
